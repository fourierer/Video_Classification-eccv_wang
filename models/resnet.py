...
class I3Res50(nn.Module):
    ...
    def forward_single(self, x, include_top):
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool1(x)
    ...
    def forward(self, input_frames, include_top=True):

        # 5D tensor == single clip
        if input_frames.dim() == 5:
            pred = self.forward_single(input_frames, include_top)

        # 7D tensor == 3 crops/10 clips
        elif input_frames.dim() == 7:
            pred = self.forward_multi(input_frames, include_top)
...
def i3_res50(num_classes, pretrained=True):
...
def i3_res50_nl(num_classes, pretrained=True):
...
if __name__=='__main__':
    net = i3_res50_nl(400, pretrained=False)
    input_frames = torch.rand(4, 3, 32, 224, 224)
    pred = net(input_frames, include_top=False)
    print(pred.shape)
